...
const historyAggregator = (results: FetchResult<QueryTableResponse>[]): QueryResponse => {
  // Juntar tudo em um único array
  const allQueries = results.flatMap((result) =>
    result.data.queries.map((q) => ({
      ...q,
      // opcional: guardar a URL de origem para debug
      sourceUrl: result.url,
    })),
  );

  // Ordenar pelo campo `time` (mais recentes primeiro)
      ...
      <Card>
        ...
        <CardContent className="flex size-full flex-col gap-4">
          <div className="overflow-hidden rounded-md border">
            <DataTableNew table={table} columns={recentQueryLogColumns} />
          </div>
