import type { WebMessages } from "./messages.types";

export const ptBRMessages: WebMessages = {
  common: {
    language: "Idioma",
    languagePlaceholder: "Selecione o idioma",
    add: "Adicionar",
    retry: "Tentar novamente",
    baseline: "Baseline",
    total: "Total",
    versionUnavailable: "N/A",
    notConfigured: "Não configurado",
    copied: "Copiado para a área de transferência",
  },

  apiUnavailable: {
    badge: "YAPD API",
    title: "Aguardando backend",
    description: "Aguardando o backend estar disponível...",
    checking: "Verificando se o backend ja esta disponivel...",
    nextAttemptIn: (seconds) => `Nova tentativa em ${seconds} ${seconds === 1 ? "segundo" : "segundos"}.`,
    attempt: (attempt) => `Tentativa automatica ${attempt}`,
    reloadNow: "Atualizar agora",
    helpDescription: "Aguarde. Você será redirecionado automaticamente...",
  },
  apiError: {
    badge: "YAPD API",
    title: "O backend respondeu com erro",
    helpTitle: "O que verificar",
    helpDescription: (apiBaseUrl, status) =>
      `Confira os logs da API, o schema do banco e tente novamente.${status ? ` Status HTTP: ${status}.` : ""} Backend: ${apiBaseUrl}.`,
  },
  external: {
    setup: {
      badge: "Setup inicial",
      title: "Defina a baseline do YAPD",
      description:
        "Esse primeiro passo registra a instância Pi-hole que vai autenticar a interface e servir como autoridade principal do produto.",
      technicalSecret:
        "O backend valida a conectividade e guarda apenas a credencial técnica da baseline de forma criptografada.",
      interactiveLogin:
        "O login humano do painel vai usar o fluxo nativo do Pi-hole v6 para gerar o SID da sessão proxy.",
      formTitle: "Configurar baseline",
      formDescription: "Preencha a instância principal do ambiente para liberar o login do produto.",
    },
    login: {
      badge: "Login da baseline",
      title: "Entre usando o Pi-hole principal",
      description: "O YAPD usa o endpoint de login do Pi-hole v6 para obter um SID e criar a sessão segura do painel.",
      ephemeralPassword: "Senha e TOTP são enviados apenas para a baseline e não ficam gravados no banco do YAPD.",
      serviceSecrets:
        "As outras instâncias seguem com segredos técnicos próprios para testes, import e operações futuras.",
    },
  },
  forms: {
    setup: {
      validation: {
        name: "Informe um nome para a baseline.",
        baseUrl: "Use uma URL completa, como https://pi.hole.",
        password: "Informe a senha ou application password do Pi-hole.",
      },
      defaults: {
        name: "Pi-hole Principal",
        baseUrl: "https://pi.hole",
      },
      toasts: {
        success: "Baseline configurada com sucesso.",
      },
      fields: {
        name: "Nome da baseline",
        baseUrl: "Base URL",
        baseUrlDescription: "Use o host principal que será a autoridade de login do produto.",
        password: "Senha/Application password",
        passwordDescription: "Essa credencial será guardada criptografada para operações técnicas do backend.",
        totp: "TOTP opcional",
        allowSelfSigned: "Permitir certificado self-signed explicitamente",
        allowSelfSignedDescription: "Use apenas quando o Pi-hole tiver um certificado local que você confia.",
        certificate: "CA personalizada opcional",
      },
      errors: {
        title: "Falha ao validar a baseline",
      },
      submit: {
        idle: "Salvar baseline",
        loading: "Validando baseline...",
      },
    },
    login: {
      validation: {
        password: "Informe a senha do Pi-hole.",
      },
      toasts: {
        success: (baselineName) => `Sessão criada via ${baselineName}.`,
      },
      fields: {
        password: "Senha do Pi-hole",
        passwordDescription: "O backend não persiste essa senha. Ela só serve para obter o SID atual da interface.",
        totp: "Código TOTP opcional",
      },
      submit: {
        idle: "Entrar no YAPD",
        loading: "Abrindo sessão...",
      },
    },
    instances: {
      validation: {
        name: "Informe um nome.",
        baseUrl: "Informe apenas host, porta e caminho opcional, sem repetir http:// ou https://.",
        password: "Informe a senha ou application password.",
        certificatePem: "Informe um certificado PEM valido.",
        trustMode: "Use CA personalizada ou self-signed explicito, mas nao os dois ao mesmo tempo.",
        candidates: "Use apenas URLs http/https validas, uma por linha ou separadas por virgula.",
        candidatesLimit: (limit) => `Informe no maximo ${limit} candidatos por descoberta.`,
      },
      toasts: {
        refreshFailed: "Não foi possível atualizar a lista de instâncias.",
        createSuccess: "Instância cadastrada com sucesso.",
        discoverSuccess: "Descoberta executada.",
        detailsLoadFailed: "Não foi possível carregar os detalhes da instância.",
        testSuccess: "Conexão validada com sucesso.",
        reauthenticateSuccess: "Sessão da instância renovada com sucesso.",
        syncEnabledSuccess: "Instância incluída novamente no sync.",
        syncDisabledSuccess: "Instância removida das operações globais de sync.",
        updateSuccess: "Instância atualizada com sucesso.",
        promotePrimarySuccess: (instanceName) => `${instanceName} agora é a instância principal.`,
      },
      create: {
        openModal: "Cadastrar instância",
        title: "Cadastrar instância",
        description: "Salve uma credencial técnica para o backend operar sobre outro Pi-hole.",
        manualTab: "Manual",
        discoveryTab: "Descoberta",
        name: "Nome",
        baseUrlScheme: "Protocolo",
        baseUrl: "Base URL",
        baseUrlPlaceholder: "pihole.lan",
        baseUrlDescription: "Informe apenas host, porta e caminho opcional. Não repita http:// ou https://.",
        password: "Senha/Application password",
        passwordDescription: "O backend vai testar a autenticação antes de salvar a instância.",
        allowSelfSigned: "Permitir self-signed explicitamente",
        certificate: "CA personalizada opcional",
        validationFailedTitle: "Falha ao validar a instância",
        cancel: "Cancelar",
        submitIdle: "Salvar instância",
        submitLoading: "Validando...",
      },
      edit: {
        title: "Editar instância",
        description: "Atualize a conexão técnica e revalide a instância antes de salvar.",
        passwordDescription: "Deixe em branco para manter a credencial técnica atual.",
        validationFailedTitle: "Falha ao atualizar a instância",
        loadFailedTitle: "Falha ao carregar a instância",
        loading: "Carregando detalhes da instância...",
        cancel: "Cancelar",
        submitIdle: "Salvar alterações",
        submitLoading: "Salvando...",
      },
      info: {
        title: "Informações da instância",
        description: (instanceName) => `Veja as informações operacionais coletadas de ${instanceName}.`,
        descriptionFallback: "Veja as informações operacionais desta instância.",
        expand: "Expandir detalhes operacionais",
        collapse: "Recolher detalhes operacionais",
        loading: "Carregando informações operacionais da instância...",
        errorTitle: "Falha ao consultar o Pi-hole",
        retry: "Tentar novamente",
        fetchedAt: "Consultado em",
        buildDate: "Build",
        summary: "Resumo",
        unavailable: "Não disponível",
        versionTab: "Version",
        hostTab: "Host",
        systemTab: "System",
        versionTitle: "Version",
        versionDescription: "Versões locais e remotas reportadas pelo endpoint info/version.",
        hostTitle: "Host",
        hostDescription: "Informações de hardware e kernel reportadas pelo endpoint info/host.",
        systemTitle: "System",
        systemDescription: "Uso de memória, CPU e uptime reportados pelo endpoint info/system.",
        memoryTitle: "Memória",
        ftlTitle: "FTL",
        core: "Core",
        web: "Web",
        ftl: "FTL",
        docker: "Docker",
        local: "Local",
        remote: "Remoto",
        model: "Modelo",
        nodename: "Hostname",
        machine: "Arquitetura",
        sysname: "Sistema",
        release: "Release",
        kernelVersion: "Kernel",
        domainname: "Domínio",
        uptime: "Uptime",
        processes: "Processos",
        cpuUsage: "CPU",
        cpuCores: "CPUs",
        cpuLoadRaw: "Load average",
        cpuLoadPercent: "Load (%)",
        ram: "RAM",
        swap: "Swap",
        total: "Total",
        free: "Livre",
        used: "Uso",
        available: "Disponível",
        percentUsed: "Uso (%)",
        ftlMemory: "FTL memória (%)",
        ftlCpu: "FTL CPU (%)",
      },
      discovery: {
        title: "Descoberta assistida",
        description: "Informe candidatos para o backend verificar se respondem como Pi-hole.",
        candidates: "Candidatos",
        candidatesDescription: (limit) =>
          `Use uma URL por linha ou separadas por vírgula, com limite de ${limit} candidatos.`,
        empty: "Nenhum resultado ainda. Rode a descoberta para testar candidatos.",
        reachable: "Pi-hole respondeu ao endpoint /auth.",
        unreachable: "Não foi possível conectar.",
        useAddressHint: "Use um resultado alcançável para preencher a Base URL na aba manual.",
        useDiscoveredAddress: "Usar este endereço",
        invalidSelection: "O resultado selecionado não contém uma URL válida.",
        submitIdle: "Executar descoberta",
        submitLoading: "Buscando...",
      },
      table: {
        title: "Instâncias cadastradas",
        description: "Teste as conexões salvas e acompanhe a baseline atual.",
        name: "Nome",
        baseUrl: "Base URL",
        trust: "Trust",
        version: "Versão",
        lastValidation: "Última validação",
        session: "Sessão",
        validUntil: "Válida até",
        sync: "Sync",
        lastError: "Último erro",
        actions: "Ações",
        baseline: "Baseline",
        baselineBadge: "Principal",
        syncEnabled: "Ativo",
        syncDisabled: "Inativo",
        syncEnabling: "Ligando...",
        syncDisabling: "Desligando...",
        syncLocked: "A baseline precisa permanecer ativa no sync.",
        errorDetails: "Mais informações do erro",
        editIdle: "Editar",
        infoIdle: "Ver informações",
        editLoading: "Abrindo...",
        makePrimaryIdle: "Tornar principal",
        makePrimaryLoading: "Alterando principal...",
        testIdle: "Testar",
        testLoading: "Testando...",
        reauthenticateIdle: "Reautenticar",
        reauthenticateLoading: "Reautenticando...",
        statusActive: "Ativa",
        statusExpired: "Expirada",
        statusMissing: "Sem SID",
        statusError: "Com erro",
      },
      promotePrimary: {
        title: "Alterar instância principal",
        description: (instanceName) =>
          `${instanceName} passará a ser a baseline principal usada globalmente pelo YAPD.`,
        warning:
          "A antiga principal perderá o status de baseline. O sync da nova principal será ativado automaticamente se necessário.",
        cancel: "Cancelar",
        confirmIdle: "Tornar principal",
        confirmLoading: "Alterando...",
      },
      errorDetails: {
        title: "Detalhes do erro da instância",
        description: (instanceName) => `Entenda com mais clareza o último problema registrado em ${instanceName}.`,
        summary: "Resumo",
        possibleCause: "Causa provável",
        whatToCheck: "O que conferir",
        technicalDetails: "Detalhe técnico",
        noTechnicalDetails: "Nenhum detalhe técnico adicional foi salvo.",
        close: "Fechar",
        invalid_credentials: {
          title: "Credenciais técnicas inválidas",
          cause: "O Pi-hole recusou a autenticação com a senha técnica salva para esta instância.",
          checks: [
            "Confirme se a application password ainda é válida.",
            "Verifique se a senha foi trocada diretamente no Pi-hole.",
            "Reautentique ou edite a instância para salvar a credencial correta.",
          ],
        },
        tls_error: {
          title: "Falha de TLS ou certificado",
          cause: "O certificado apresentado por esta instância não foi aceito pela configuração atual de trust.",
          checks: [
            "Confirme se o endereço usa o protocolo correto.",
            "Se for certificado próprio, habilite self-signed explicitamente ou envie a CA personalizada.",
            "Verifique se o certificado do servidor mudou recentemente.",
          ],
        },
        timeout: {
          title: "Tempo de conexão esgotado",
          cause: "O backend não conseguiu concluir a comunicação com o Pi-hole dentro do tempo limite configurado.",
          checks: [
            "Confirme se a instância está online e acessível pela rede do backend.",
            "Verifique se host, porta e caminho estão corretos.",
            "Cheque latência alta, proxy reverso ou firewall no caminho.",
          ],
        },
        dns_error: {
          title: "Hostname não resolvido",
          cause: "O hostname informado na Base URL não conseguiu ser resolvido para um IP válido.",
          checks: [
            "Revise o hostname digitado.",
            "Teste usar o IP diretamente para confirmar o alcance.",
            "Garanta que o backend tenha DNS funcional para esse endereço.",
          ],
        },
        connection_refused: {
          title: "Conexão recusada",
          cause: "Houve resposta da rede, mas a porta de destino recusou a conexão.",
          checks: [
            "Confirme se o Pi-hole está em execução na porta esperada.",
            "Verifique se há proxy, NAT ou firewall bloqueando a porta.",
            "Valide se o protocolo selecionado corresponde ao serviço exposto.",
          ],
        },
        pihole_response_error: {
          title: "Resposta inesperada do Pi-hole",
          cause: "A instância respondeu, mas em um formato diferente do esperado para a operação executada.",
          checks: [
            "Confirme se o endereço realmente aponta para a API do Pi-hole v6.",
            "Verifique se há proxy ou página intermediária alterando a resposta.",
            "Teste a instância novamente depois de confirmar versão e rota.",
          ],
        },
        unknown: {
          title: "Falha não classificada",
          cause: "O backend encontrou um erro que não entrou em uma categoria mais específica.",
          checks: [
            "Revise a Base URL e a conectividade da instância.",
            "Confirme credenciais, TLS e disponibilidade do serviço.",
            "Use o detalhe técnico abaixo para identificar o ponto exato da falha.",
          ],
        },
      },
      page: {
        eyebrow: "Infraestrutura gerenciada",
        title: "Instâncias",
        totalDescription: (count) => `${count} instâncias`,
        totalCaption: "Incluindo a baseline principal.",
      },
    },
  },
  layout: {
    actionsMenuLabel: "Ações rápidas",
    overviewButton: "Visão geral",
    queriesButton: "Queries",
    instancesButton: "Instâncias",
    beta: {
      label: "BETA",
      openLabel: "Abrir aviso da versão BETA",
      title: "YAPD em desenvolvimento",
      description:
        "O YAPD - Yet Another Pi-hole Dashboard é um projeto independente de terceiros em fase de desenvolvimento.",
      disclaimer:
        "O YAPD não é afiliado, endossado nem patrocinado pela Pi-hole, LLC. Não sou responsável por problemas que possam ser causados em suas instâncias Pi-hole de produção.",
      close: "Entendi",
    },
  },
  sidebar: {
    groups: {
      overview: "Visão geral",
      operations: "Operações",
      status: "Status",
      sync: "Sync",
    },
    items: {
      dashboard: "Dashboard",
      queries: "Queries Log",
      groups: "Grupos",
      clients: "Clientes",
      domains: "Domínios",
      browserExtension: "YAPD Inspector",
      lists: "Ad-lists",
      instances: "Instâncias",
      config: "Configurações",
      notifications: "Notificações",
      baselineLogin: "Login da baseline",
      setupBaseline: "Setup da baseline",
      apiHealth: "Saúde da API",
    },
    search: {
      button: "Acesso rápido",
      placeholder: "Encontre rotas e ações…",
      empty: "Nenhum resultado encontrado.",
      group: "Workspace",
    },
    user: {
      logoutError: "Não foi possível encerrar a sessão.",
      role: "Operador da baseline",
      proxy: "Sessão proxy",
      authority: "Autoridade Pi-hole",
      logout: "Sair",
    },
    statusCard: {
      title: "Status do slice 1",
      description: "Setup, login e gerenciamento de instâncias já estão conectados para a primeira entrega do YAPD.",
    },
    controls: {
      title: "Preferências",
      description: "Personalize o idioma, o timezone e o layout do seu painel.",
      language: "Idioma",
      timeZone: "Timezone",
      timeZonePlaceholder: "Selecione ou pesquise um timezone",
      timeZoneEmpty: "Nenhum timezone encontrado.",
      timeZoneInvalid: "Informe um timezone IANA válido.",
      themePreset: "Preset do tema",
      font: "Fonte",
      fontPlaceholder: "Selecione a fonte",
      themeMode: "Modo do tema",
      light: "Claro",
      dark: "Escuro",
      system: "Sistema",
      pageLayout: "Layout da página",
      centered: "Centralizado",
      fullWidth: "Largura total",
      navbarBehavior: "Comportamento da barra",
      sticky: "Fixa",
      scroll: "Rolagem",
      sidebarStyle: "Estilo da sidebar",
      inset: "Inset",
      sidebar: "Sidebar",
      floating: "Flutuante",
      sidebarCollapseMode: "Modo de recolhimento",
      icon: "Ícone",
      offcanvas: "Offcanvas",
      restore: "Restaurar padrões",
    },
    sync: {
      title: "Sync",
      blocking: "Blocking",
      statusTitle: "Estado atual",
      disableBlocking: "Disable Blocking",
      enableBlocking: "Reabilitar blocking",
      refresh: "Atualizar",
      loading: "Carregando status...",
      refreshLoading: "Atualizando...",
      quickIndefinite: "Indefinido",
      quick10Seconds: "10 segundos",
      quick30Seconds: "30 segundos",
      quick5Minutes: "5 minutos",
      savedPreset: "Preset salvo",
      configurePreset: "Gerenciar presets",
      custom: "Customizado",
      missingPreset: "Nenhum preset salvo ainda.",
      aggregateEnabled: "Enabled",
      aggregateDisabled: "Disabled",
      aggregateMixed: "Mixed",
      aggregatePartial: "Partial",
      instanceEnabled: "Enabled",
      instanceDisabled: "Disabled",
      instanceUnavailable: "Sem leitura",
      timerLabel: "Timer",
      messageLabel: "Motivo",
      valueLabel: "Valor",
      unitLabel: "Unidade",
      unitSeconds: "Segundos",
      unitMinutes: "Minutos",
      invalidValue: "Informe um valor inteiro positivo.",
      normalizedTimer: (timerLabel) => `Será enviado como ${timerLabel}.`,
      presetDescription: "Use presets salvos para aplicar tempos recorrentes com um clique.",
      presetsManagerTitle: "Gerenciar presets",
      presetsManagerDescription: "Reordene, renomeie, crie e remova os presets de duração do blocking.",
      presetAliasLabel: "Alias do preset",
      addPreset: "Adicionar preset",
      removePreset: "Apagar",
      movePresetUp: "Mover para cima",
      movePresetDown: "Mover para baixo",
      emptyPresets: "Nenhum preset configurado.",
      savePresets: "Salvar presets",
      savingPresets: "Salvando presets...",
      customDescription: "Informe uma duração personalizada. O frontend sempre converte para segundos antes do envio.",
      previewTitle: "Confirmar sincronismo",
      readyInstances: (count) => `${count} prontas`,
      noopInstances: (count) => `${count} sem alteração`,
      failedInstances: (count) => `${count} com falha`,
      noReadyInstances: "Nenhuma instância pronta para aplicar agora.",
      desiredEnable: "Reabilitar blocking em todas as instâncias prontas.",
      desiredDisable: (timerLabel) => `Desabilitar blocking por ${timerLabel} nas instâncias prontas.`,
      applyReady: (count) => `Aplicar em ${count} instâncias prontas`,
      applyPartial: (count) => `Aplicar parcialmente em ${count} instâncias`,
      savePreset: "Salvar preset",
      savingPreset: "Salvando...",
      previewing: "Validando...",
      applying: "Aplicando...",
      cancel: "Cancelar",
      close: "Fechar",
      statusRefreshError: "Não foi possível atualizar o status do blocking.",
      toasts: {
        presetSaved: "Presets de blocking salvos com sucesso.",
        alreadyDesired: "As instâncias já estavam no estado desejado.",
        applySuccess: (count) =>
          count === 1 ? "Blocking sincronizado em 1 instância." : `Blocking sincronizado em ${count} instâncias.`,
        applyPartial: (successfulCount, failedCount) =>
          `${successfulCount} instâncias foram atualizadas e ${failedCount} falharam durante o sincronismo.`,
        applyFailure: "Não foi possível aplicar o sincronismo de blocking.",
        instanceFailure: (instanceName, message) => `${instanceName}: ${message}`,
        masterMismatch: (instanceName, instanceAddress) =>
          `${instanceName} (${instanceAddress}) não está com o mesmo estado de blocking da instância master.`,
      },
    },
  },
  dashboard: {
    eyebrow: "Visão consolidada das instâncias Pi-hole",
    title: "Dashboard",
    scope: {
      label: "Escopo",
      allInstances: "Todas as instâncias",
      placeholder: "Selecione uma instância",
    },
    cards: {
      totalQueries: "Total Queries",
      queriesBlocked: "Queries Bloqueadas",
      percentageBlocked: "% Bloqueados",
      domainsOnList: "Total de dominios em ad-lists",
    },
    charts: {
      totalQueriesTitle: "Total Queries",
      totalQueriesDescription: (count) =>
        count === 1
          ? "Últimas 24 horas da instância selecionada, em janelas de 1 hora."
          : `Últimas 24 horas agregadas entre ${count} instâncias, em janelas de 1 hora.`,
      clientActivityTitle: "Atividade de clientes",
      clientActivityDescription: (count) =>
        count === 1
          ? "Clientes da instância selecionada nas últimas 24 horas, em janelas de 1 hora."
          : `Top clientes somados entre ${count} instâncias nas últimas 24 horas, em janelas de 1 hora.`,
      noData: "Sem dados suficientes para montar este gráfico ainda.",
    },
    partial: {
      title: "Dashboard parcial",
      description: (failedCount, totalCount) =>
        failedCount === 1
          ? `1 de ${totalCount} instância falhou ao carregar métricas. Os dados abaixo somam apenas as instâncias saudáveis.`
          : `${failedCount} de ${totalCount} instâncias falharam ao carregar métricas. Os dados abaixo somam apenas as instâncias saudáveis.`,
    },
    toasts: {
      instanceFailure: (instanceName, message) => `${instanceName}: ${message}`,
      genericInstanceFailure: (instanceName) =>
        `${instanceName}: não foi possível carregar as métricas desta instância.`,
    },
  },
  notifications: {
    eyebrow: "Eventos persistidos",
    title: "Notificações",
    description: "Acompanhe mensagens do Pi-hole e falhas operacionais recentes em um só lugar.",
    preview: {
      title: "Últimas notificações",
      empty: "Nenhuma notificação não lida.",
      markAllRead: "Marcar todas como lidas",
      seeAll: "Ver todas",
      unreadCount: (count) => (count === 1 ? "1 não lida" : `${count} não lidas`),
      pushEnable: "Ativar push",
      pushDisable: "Desativar push",
      pushEnabled: "Push ativo neste dispositivo",
      pushDenied: "Permissão de push bloqueada no navegador",
      pushUnsupported: "Push não suportado neste navegador",
      pushInsecureContext: "Push exige HTTPS ou localhost",
      pushServerUnavailable: "Configuração de push indisponível",
      pushEnabling: "Ativando push...",
    },
    table: {
      tabs: {
        unread: "Não lidas",
        read: "Lidas",
      },
      type: "Tipo",
      instance: "Instância",
      message: "Mensagem",
      time: "Hora",
      actions: "Ações",
      rowsPerPage: "Linhas por página",
      showing: (start, end, total) => `Mostrando ${start}-${end} de ${total}`,
      previous: "Anterior",
      next: "Próxima",
      emptyUnreadTitle: "Sem notificações não lidas",
      emptyUnreadDescription: "Novos eventos aparecerão aqui assim que forem encontrados.",
      emptyReadTitle: "Sem notificações lidas",
      emptyReadDescription: "Quando você marcar notificações como lidas, elas aparecerão aqui.",
      unreadBadge: (count) => (count === 1 ? "1 não lida" : `${count} não lidas`),
      readBadge: (count) => (count === 1 ? "1 lida" : `${count} lidas`),
      systemInstance: "Sistema",
    },
    actions: {
      markRead: "Marcar como lida",
      markAllRead: "Marcar todas como lidas",
      delete: "Apagar",
      deleting: "Apagando...",
      markingRead: "Marcando...",
    },
    toasts: {
      markReadSuccess: "Notificação marcada como lida.",
      deleteSuccess: "Notificação ocultada com sucesso.",
      markAllReadSuccess: (count) =>
        count === 1 ? "1 notificação marcada como lida." : `${count} notificações marcadas como lidas.`,
      pushEnabled: "Push ativado neste dispositivo.",
      pushDisabled: "Push desativado neste dispositivo.",
      pushUnsupported: "Este navegador não suporta notificações push.",
      pushInsecureContext: "Notificações push exigem HTTPS ou localhost. Acesse o YAPD por um domínio HTTPS.",
      pushServerUnavailable: "A configuração de push ainda não está disponível no servidor.",
      pushDenied: "Permita notificações no navegador para ativar o push.",
      pushPermissionBlocked:
        "A permissão de notificações está bloqueada neste site. Libere nas configurações do navegador e tente novamente.",
      pushServiceWorkerUnavailable:
        "O Service Worker de notificações não ficou pronto. Recarregue a página e tente novamente.",
      pushSubscribeFailed: "O navegador recusou a inscrição push. Verifique as permissões do site e tente novamente.",
      pushSubscriptionSaveFailed: "O navegador criou a inscrição, mas o YAPD não conseguiu salvá-la no servidor.",
      pushSessionExpired: "Sua sessão expirou. Entre novamente e tente ativar o push.",
      pushSecurityRejected:
        "O servidor recusou a ativação por token de segurança inválido. Recarregue a página e tente novamente.",
      pushFailed: "Não foi possível configurar o push agora.",
      refreshFailed: "Não foi possível atualizar as notificações.",
    },
    types: {
      RATE_LIMIT: "Rate limit",
      CONNECTION_ERROR: "Erro de conexão",
      CLIENTS_FAILURE: "Falha em clientes",
      DOMAINS_FAILURE: "Falha em domínios",
      GROUPS_FAILURE: "Falha em grupos",
      INSTANCES_FAILURE: "Falha em instâncias",
      LISTS_FAILURE: "Falha em listas",
      NOTIFICATION_SYNC_ERROR: "Falha no coletor",
      INSTANCE_SESSION_ERROR: "Falha de sessão",
      SYNC_FAILURE: "Falha de sincronismo",
      SYSTEM_FAILURE: "Falha de sistema",
      OVERVIEW_IMPORT_SUCCESS: "Importação do overview concluída",
      OVERVIEW_IMPORT_PARTIAL: "Importação parcial do overview",
      OVERVIEW_IMPORT_FAILURE: "Falha na importação do overview",
      OVERVIEW_DELETE_SUCCESS: "Exclusão do overview concluída",
      OVERVIEW_DELETE_FAILURE: "Falha na exclusão do overview",
      OVERVIEW_COVERAGE_RENEWED: "Cobertura do overview renovada",
      unknown: (type) => type,
    },
    failureTitles: {
      invalid_credentials: "Credenciais inválidas",
      tls_error: "Erro TLS",
      timeout: "Timeout de conexão",
      dns_error: "Erro de DNS",
      connection_refused: "Conexão recusada",
      pihole_response_error: "Erro retornado pelo Pi-hole",
      unknown: "Falha ao alcançar a instância",
    },
  },
  overview: {
    eyebrow: "Histórico consolidado do banco local",
    title: "Overview",
    description:
      "Consulte o histórico salvo no banco local, acompanhe a cobertura disponível e solicite coletas ou exclusões em segundo plano.",
    tour: {
      open: "Abrir tour do Overview",
      close: "Fechar tour",
      next: "Próximo",
      finish: "Concluir",
      purpose:
        "O Overview consolida o histórico salvo localmente e separa consulta, rankings e jobs para acompanhar coletas sem bloquear a tela.",
      tabs: "Use as abas para alternar entre solicitação de período, análise dos rankings e acompanhamento dos jobs.",
      manualCollection:
        "Aqui você escolhe um dia fechado e enfileira uma coleta histórica para preencher o banco local.",
      coverage:
        "A cobertura mostra o que já está salvo, quando expira e quais períodos podem ser renovados sem nova coleta.",
      rankingFilters:
        "As datas salvas e os filtros do Ranking controlam o universo usado nos gráficos e tabelas abaixo.",
      rankingResults:
        "Os rankings e gráficos são clicáveis quando faz sentido; clicar em domínios ou clientes aplica o filtro e recarrega o período.",
      jobs: "A lista de jobs mostra coletas, exclusões, progresso e detalhes operacionais para investigar falhas ou retomar execuções. Também existe uma coleta automática todos os dias às 03:00 no timezone configurado na aplicação.",
    },
    tabs: {
      request: "Período e cobertura",
      ranking: "Ranking",
      jobs: "Jobs",
      settings: "Configurações",
    },
    filters: {
      title: "Solicitação de período",
      target: "Instância",
      allInstancesTarget: "Todas as instâncias",
      date: "Data",
      from: "De",
      until: "Até",
      closedDayHint:
        "A coleta manual aceita somente um dia fechado por solicitação. A data atual fica bloqueada, mas você pode ajustar a hora dentro do dia selecionado.",
      timeZoneHint: (timeZone) => `Período avaliado no timezone configurado da aplicação: ${timeZone}.`,
      apply: "Aplicar filtros",
      applying: "Aplicando...",
      reload: "Recarregar",
    },
    actions: {
      backfill: "Solicitar coleta",
      backfillLoading: "Enfileirando...",
      deletePeriod: "Apagar período",
      deletePeriodLoading: "Enfileirando...",
    },
    partial: {
      title: "Dados parciais",
      description:
        "Algumas instâncias não têm histórico disponível para esse período. Os rankings e gráficos abaixo usam apenas o que está salvo localmente.",
      missingDataMessage: "Sem histórico salvo para o período selecionado.",
      importFailureMessage: (message) => `Falha na coleta anterior: ${message}`,
    },
    summary: {
      totalQueries: "Total no período",
      blockedQueries: "Bloqueadas",
      uniqueClients: "Clientes únicos",
      percentageBlocked: "Taxa de bloqueio",
    },
    chart: {
      titleByHour: "Volume por hora",
      titleByDay: "Volume por dia",
      description: "Leitura consolidada apenas do banco histórico local.",
      noDataTitle: "Sem dados no período",
      noDataDescription: "Solicite uma coleta histórica ou ajuste os filtros para um intervalo já salvo.",
    },
    coverage: {
      title: "Cobertura disponível",
      description: "Períodos salvos para o escopo selecionado, com destaque para o que está perto de expirar.",
      totalStoredQueries: "Queries salvas",
      earliestStoredAt: "Primeiro registro",
      latestStoredAt: "Último registro",
      savedWindows: "Períodos salvos",
      expiringSoon: "Próximos da expiração",
      unavailable: "N/A",
      emptyTitle: "Sem cobertura salva",
      emptyDescription: "Nenhuma coleta concluída foi encontrada para o escopo atual.",
      completedEmptyTitle: "Nenhum período concluído",
      completedEmptyDescription: "Somente períodos com status Sucesso ou Parcial aparecem nesta lista.",
      capturedQueries: (count) => (count === 1 ? "1 query capturada" : `${count} queries capturadas`),
      period: (from, until) => `${from} até ${until}`,
      expiresAt: (value) => `Expira em ${value}`,
      renew: "Renovar +30 dias",
      renewing: "Renovando...",
      expiringSoonTitle: "Cobertura perto da expiração",
      expiringSoonDescription: (count) =>
        `${count} período(s) salvo(s) vencem em breve e podem ser renovados sem nova coleta.`,
      expiringSoonBadge: "Expira em breve",
      paginationPrevious: "Anterior",
      paginationNext: "Próximo",
      paginationStatus: (page, totalPages) => `Página ${page} de ${totalPages}`,
    },
    ranking: {
      title: "Rankings",
      description: "Resumo dos dados salvos localmente para o período selecionado.",
      filtersTitle: "Filtro do ranking",
      filtersDescription: "Ajuste aqui o período usado pelos rankings e gráficos desta aba.",
      savedDatesTitle: "Datas salvas",
      savedDatesDescription: "Escolha o intervalo de dias salvos usado nos rankings e gráficos.",
      emptySavedDatesTitle: "Nenhuma data salva",
      emptySavedDatesDescription: "Solicite uma coleta ou escolha outro escopo para listar dias com histórico local.",
      savedDateSelectLabel: "Data salva",
      savedDateFromLabel: "Data inicial",
      savedDateUntilLabel: "Data final",
      savedDateSelectPlaceholder: "Selecione um dia salvo",
      savedDateOption: (date, rowCount, instanceCount) =>
        `${date} | ${rowCount} queries | ${instanceCount} instância(s)`,
      periodFilter: "Período",
      savedDateLegend: "Dias com ponto verde têm registro salvo.",
      noSavedDateLegend: "Nenhuma data salva para destacar neste escopo.",
      domainFilter: "Domínio",
      domainPlaceholder: "example.com",
      clientFilter: "IP do cliente",
      allClients: "Todos os clientes",
      groupBy: "Agrupar por",
      groupByValues: {
        hour: "Hora",
        day: "Dia",
      },
      applyFilters: "Aplicar",
      clearFilters: "Limpar",
      periodActions: "Ações do período",
      backToPreviousPeriod: "Voltar ao período anterior",
      drillDownLoading: "Carregando período...",
      deletePeriodDialogTitle: "Apagar período selecionado?",
      deletePeriodDialogDescription: (period) =>
        `Esta ação cria um job para apagar os dados salvos entre ${period}. Os rankings ficarão indisponíveis para esse intervalo até uma nova coleta.`,
      deletePeriodDialogConfirm: "Apagar período",
      deletePeriodDialogCancel: "Cancelar",
      nativeWindowNoticeTitle: "Diferença de janela entre Pi-hole e YAPD",
      nativeWindowNoticeDescription:
        "O dashboard nativo do Pi-hole costuma processar datas até o minuto 55. O YAPD considera o período completo até 23:59:59, então os totais podem divergir.",
      statusDistributionTitle: "Distribuição por status",
      statusDistributionDescription: "Participação dos principais status no universo filtrado.",
      kpis: {
        peakQueries: "Pico do período",
        peakBlocked: "Pico de bloqueio",
        topDomain: "Domínio líder",
        topClient: "Cliente líder",
        topUpstream: "Upstream líder",
        topStatus: "Status líder",
        noValue: "Sem dados",
      },
      analytics: {
        contextTitle: "Universo analisado",
        filteredContextTitle: "Universo filtrado",
        contextDescription: (from, until) =>
          `Período de ${from} até ${until}. Percentuais calculados sobre este conjunto.`,
        clientContext: (clientIp) => `Cliente: ${clientIp}`,
        domainContext: (domain) => `Domínio: ${domain}`,
        groupContext: (group) => `Agrupamento: ${group}`,
        totalQueries: "Total",
        allowedQueries: "Aceitas",
        blockedQueries: "Bloqueadas",
        blockedPercentage: "% bloqueada",
        topDomainsShareTitle: "Participação dos domínios",
        topDomainsShareDescription: "Top domínios e participação sobre o universo filtrado.",
        topClientsShareTitle: "Clientes por volume",
        topClientsShareDescription: "Volume e participação dos clientes no período selecionado.",
        hourlyAccessTitle: "Horários com mais acessos",
        hourlyAccessDescription: "Soma por hora do dia no período selecionado.",
        hourlyAccessUnavailable: "Use o agrupamento por hora para visualizar a distribuição horária.",
        hourlyAccessPeak: (hour, count) => `Maior volume em ${hour}: ${count} queries.`,
        domainColumn: "Domínio",
        clientColumn: "Cliente",
        totalColumn: "Total",
        percentageColumn: "%",
        other: "Outros",
      },
      domains: "Top domínios",
      clients: "Top clientes",
      upstreams: "Top upstreams",
      showUpstreams: "Mostrar upstreams",
      hideUpstreams: "Ocultar upstreams",
      statuses: "Top status",
      value: "Valor",
      queries: "Queries",
      noData: "Nenhum dado salvo para o período atual.",
    },
    jobs: {
      title: "Jobs",
      description: "Atualizações e exclusões em segundo plano.",
      refresh: "Atualizar jobs",
      refreshing: "Atualizando...",
      emptyTitle: "Nenhum job encontrado",
      emptyDescription: "As próximas coletas e exclusões aparecerão aqui.",
      filteredEmptyTitle: "Nenhum job neste filtro",
      filteredEmptyDescription: "Ajuste os filtros de status para ver outros jobs.",
      statusFilterTitle: "Filtrar por status",
      statusFilterValues: {
        all: "Todos",
        inProgress: "Em execução",
        completed: "Concluídos",
        partial: "Parcial",
        failure: "Falha",
        cancelled: "Cancelados",
      },
      status: "Status",
      type: "Tipo",
      instance: "Instância",
      period: "Período",
      rows: "Linhas",
      progress: "Progresso",
      actions: "Ações",
      openPeriod: "Abrir período",
      viewDetails: "Ver detalhes",
      retry: "Tentar novamente",
      cancel: "Cancelar",
      delete: "Apagar job",
      cancelJobDialogTitle: "Cancelar este job?",
      cancelJobDialogDescription: (kind, status, period) =>
        `Esta ação solicita o cancelamento do job ${kind} com status ${status} no período ${period}. Se ele já estiver em execução, o processamento será interrompido no próximo ponto seguro e os dados parciais desse job serão removidos.`,
      cancelJobDialogConfirm: "Cancelar job",
      cancelJobDialogCancel: "Voltar",
      deleteJobDialogTitle: "Apagar este job?",
      deleteJobDialogDescription: (kind, status, period) =>
        `Esta ação apaga o job ${kind} com status ${status} e remove os dados históricos vinculados ao período ${period}.`,
      deleteJobDialogConfirm: "Apagar job",
      deleteJobDialogCancel: "Cancelar",
      runningHint: "Aguardando conclusão",
      progressSummary: (saved, expected) => `${saved} salvas de ${expected} esperadas`,
      pagesSummary: (completed, total) => `${completed} de ${total} páginas`,
      etaCalculating: "ETA: calculando...",
      etaRemaining: (value) => `ETA: ~${value}`,
      elapsedDuration: (value) => `Decorrido: ${value}`,
      detailsTitle: "Detalhes do job",
      detailsDescription: (from, until) => `Timeline completa da execução entre ${from} e ${until}.`,
      detailsLoading: "Carregando detalhes do job...",
      detailsLive: "Ao vivo",
      detailsUpdatedAt: (value) => `Atualizado em ${value}`,
      detailsTourDemoBadge: "Demonstração",
      detailsTourDemoTitle: "Demonstração do tour",
      detailsTourDemoDescription:
        "Este modal foi aberto pelo tour para mostrar onde acompanhar progresso, instâncias e timeline do job d-1 criado agora.",
      detailsTabSummary: "Resumo",
      detailsTabInstances: "Instâncias",
      detailsTabTimeline: "Timeline",
      detailsScope: "Escopo",
      detailsAllInstances: "Todas as instâncias",
      detailsOrigin: "Origem",
      detailsOriginAutomatic: "Importação automática",
      detailsOriginManual: "Importação manual",
      detailsPeriodLabel: "Período",
      detailsRecordsFiltered: "recordsFiltered",
      detailsTotalPages: "Total de páginas",
      detailsExpected: "Total esperado",
      detailsSaved: "Total salvo",
      detailsAttempts: "Tentativas",
      detailsProgressTitle: "Progresso consolidado",
      detailsCompletedPercentage: "% concluído",
      detailsExecutionTitle: "Execução",
      detailsCreatedAt: "Criado em",
      detailsStartedAt: "Iniciado em",
      detailsFinishedAt: "Finalizado em",
      detailsUnavailable: "N/A",
      detailsFailureTitle: "Motivo principal da falha",
      detailsFailureInstance: (instanceName) => `Instância: ${instanceName}`,
      detailsFailurePage: (page) => `Página: ${page}`,
      detailsRetryAt: (value) => `Retentativa: ${value}`,
      detailsLastSuccessTitle: "Última etapa com sucesso",
      detailsLastSuccessInstance: (instanceName) => `Instância: ${instanceName}`,
      detailsLastSuccessPage: (page) => `Página: ${page}`,
      detailsLastSuccessAt: (value) => `Concluída em: ${value}`,
      detailsStalledTitle: "Execução travada em",
      detailsStalledInstance: (instanceName) => `Instância: ${instanceName}`,
      detailsStalledPage: (page) => `Página: ${page}`,
      detailsStalledStart: (value) => `Offset start: ${value}`,
      detailsCheckpointTitle: "Checkpoint atual",
      detailsCheckpointInstance: "Instância",
      detailsCheckpointPage: "Página",
      detailsCheckpointStart: "Offset start",
      detailsCheckpointFailures: "Falhas consecutivas",
      detailsNoCheckpoint: "Nenhum checkpoint ativo.",
      detailsInstancesTitle: "Progresso por instância",
      detailsTimelineTitle: "Timeline de eventos",
      detailsNoTimeline: "Nenhum evento registrado para este job.",
      failureReason: {
        timeout: "Timeout",
        session: "Erro de sessão",
        serverUnavailable: "Servidor indisponível",
        countMismatch: "Divergência de contagem",
        unexpected: "Falha inesperada",
      },
      statusValues: {
        PENDING: "Na fila",
        RUNNING: "Executando",
        PAUSED: "Pausado",
        CANCELLED: "Cancelado",
        SUCCESS: "Sucesso",
        PARTIAL: "Parcial",
        FAILURE: "Falha",
      },
      statusSummary: {
        PENDING: "Aguardando na fila.",
        RUNNING: "Coleta em andamento com progresso parcial salvo.",
        PAUSED: "Execução pausada após falhas consecutivas.",
        CANCELLED: "Cancelado antes ou durante a execução.",
        SUCCESS: "Período concluído com resultado utilizável.",
        PARTIAL: "Período concluído com lacunas em parte das instâncias.",
        FAILURE: "Execução encerrada sem resultado utilizável.",
      },
      kindValues: {
        AUTOMATIC_IMPORT: "Importação automática",
        MANUAL_IMPORT: "Importação manual",
        MANUAL_DELETE: "Exclusão manual",
      },
    },
    settings: {
      formTitle: "Regra de importação automática",
      formDescription: (timeZone) => `Agendamento avaliado no timezone configurado na aplicação: ${timeZone}.`,
      rulesTitle: "Importações automáticas",
      rulesDescription: "Regras salvas no banco para enfileirar coletas históricas.",
      name: "Nome",
      namePlaceholder: "Coleta diária principal",
      enabled: "Ativa",
      enabledDescription: "Regras inativas ficam salvas, mas não agendam jobs.",
      target: "Instância",
      allInstances: "Todas as instâncias",
      missingInstance: "Instância indisponível",
      preset: "Preset",
      presets: {
        daily3: "Todos os dias às 03:00",
        daily0: "Todos os dias às 00:00",
        daily6: "Todos os dias às 06:00",
        hourly: "A cada hora",
        custom: "Expressão personalizada",
      },
      hour: "Hora",
      minute: "Minuto",
      days: "Dias",
      dayValues: {
        everyDay: "Todos os dias",
        weekdays: "Dias úteis",
        weekends: "Fins de semana",
      },
      applyBuilder: "Aplicar",
      cronExpression: "Cron",
      fixedWindowNotice: (timeZone) =>
        `O cron define quando roda; a coleta automática continua importando o d-1 completo no timezone visual ${timeZone}.`,
      create: "Criar regra",
      update: "Salvar regra",
      saving: "Salvando...",
      cancelEdit: "Cancelar edição",
      refresh: "Atualizar",
      edit: "Editar",
      delete: "Apagar",
      actions: "Ações da regra",
      enableAction: "Ativar",
      disableAction: "Desativar",
      enabledStatus: "Ativa",
      disabledStatus: "Inativa",
      nextRun: "Próxima execução",
      nextRunDisabled: "Regra desativada; sem próxima execução.",
      lastRun: "Última execução",
      effectiveTimeZone: "Timezone efetivo",
      neverRun: "Ainda não executou",
      lastRunSummary: (status, at, jobCount, skippedCount) =>
        `${status} em ${at}; ${jobCount} job(s), ${skippedCount} pulado(s).`,
      runStatus: {
        SUCCESS: "Sucesso",
        SKIPPED: "Pulado",
        FAILURE: "Falha",
      },
      emptyTitle: "Nenhuma regra configurada",
      emptyDescription: "Crie uma regra para agendar coletas automáticas do d-1.",
      deleteDialogTitle: "Apagar regra automática?",
      deleteDialogDescription: (name) => `A regra "${name}" deixará de agendar novas coletas automáticas.`,
      deleteDialogConfirm: "Apagar regra",
      deleteDialogCancel: "Cancelar",
    },
    notifications: {
      triggerAutomatic: "automática",
      triggerManual: "manual",
      triggerUnknown: "executada",
      importSuccessMessage: (trigger, from, until, queryCount) =>
        `${trigger} entre ${from} e ${until} concluída com ${queryCount} queries importadas.`,
      importPartialMessage: (trigger, from, until, failedCount) =>
        `${trigger} entre ${from} e ${until} concluída parcialmente com falha em ${failedCount} instância(s).`,
      importFailureMessage: (trigger, from, until, errorMessage) =>
        errorMessage?.trim().length
          ? `${trigger} entre ${from} e ${until} falhou: ${errorMessage}`
          : `${trigger} entre ${from} e ${until} falhou.`,
      deleteSuccessMessage: (from, until, deletedCount) =>
        `Exclusão entre ${from} e ${until} concluída com ${deletedCount} registro(s) removidos.`,
      deleteFailureMessage: (from, until, errorMessage) =>
        errorMessage?.trim().length
          ? `Exclusão entre ${from} e ${until} falhou: ${errorMessage}`
          : `Exclusão entre ${from} e ${until} falhou.`,
      coverageRenewedMessage: (from, until, expiresAt) =>
        `Cobertura entre ${from} e ${until} renovada até ${expiresAt}.`,
    },
    toasts: {
      invalidPeriod: "Informe um período válido antes de iniciar a operação.",
      rankingPeriodWithoutCoverage: "Nenhuma data salva cruza o período escolhido.",
      instanceFailure: (instanceName, message) => `${instanceName}: ${message}`,
      jobsRefreshFailed: "Não foi possível atualizar os jobs do overview.",
      backfillQueued: (count) =>
        count === 1
          ? "1 job de coleta enfileirado ou reaproveitado."
          : `${count} jobs de coleta enfileirados ou reaproveitados.`,
      deleteQueued: "Job de exclusão enfileirado.",
      backfillFailed: "Não foi possível solicitar a coleta histórica.",
      deleteFailed: "Não foi possível solicitar a exclusão do período.",
      retryQueued: "Novo job enfileirado a partir da tentativa anterior.",
      retryFailed: "Não foi possível reenfileirar esse job.",
      cancelled: "Job cancelado.",
      cancelFailed: "Não foi possível cancelar esse job.",
      jobDeleted: "Job removido da lista com sucesso.",
      jobDeleteFailed: "Não foi possível apagar esse job.",
      jobDetailsFailed: "Não foi possível carregar os detalhes desse job.",
      currentDayBlocked:
        "O overview histórico não permite usar a data atual. Escolha no máximo o dia anterior fechado.",
      coverageRenewed: (renewedQueryCount) => `Cobertura renovada por mais 30 dias para ${renewedQueryCount} query(s).`,
      coverageRenewFailed: "Não foi possível renovar essa cobertura.",
      tourCompletionFailed: "Não foi possível salvar que o tour foi concluído.",
      automaticImportsRefreshFailed: "Não foi possível atualizar as regras automáticas.",
      automaticImportSaved: "Regra automática salva.",
      automaticImportSaveFailed: "Não foi possível salvar a regra automática.",
      automaticImportDeleted: "Regra automática apagada.",
      automaticImportDeleteFailed: "Não foi possível apagar a regra automática.",
    },
  },
  queries: {
    eyebrow: "Logs compartilhados entre instâncias Pi-hole",
    title: "Queries",
    description: "Acompanhe as consultas DNS mais recentes com filtros, sugestões e atualização contínua.",
    responsiveInstances: (successfulCount, totalCount) =>
      totalCount === 0
        ? "Nenhuma instância cadastrada."
        : successfulCount === 1
          ? "1 instância contribuiu com dados nesta leitura."
          : `${successfulCount} instâncias contribuíram com dados nesta leitura.`,
    liveInterval: "Live update a cada 2 segundos.",
    filters: {
      title: "Filtros",
      description: "Combine período, origem e tipos de resposta para recortar a tabela.",
      show: "Mostrar filtros",
      hide: "Ocultar filtros",
      from: "De",
      until: "Até",
      length: "Quantidade",
      domain: "Domínio",
      clientIp: "Client IP",
      groups: "Grupos",
      upstream: "Upstream",
      type: "Tipo",
      status: "Status",
      reply: "Reply",
      dnssec: "DNSSEC",
      disk: "Buscar no banco em disco",
      diskDescription:
        "Consulta dados on-disk. Isso é bem mais lento, mas necessário se você quiser obter queries com mais de 24 horas. Esta opção desativa o live update.",
      apply: "Aplicar filtros",
      applying: "Aplicando...",
      clear: "Limpar",
      timeZoneHint: (timeZone) => `Os filtros de período usam o timezone ${timeZone}.`,
      suggestionsLoading: "Carregando sugestões...",
      suggestionPlaceholder: "Digite ou escolha da lista",
      empty: "Nenhuma opção encontrada.",
    },
    table: {
      title: "Tabela de queries",
      description: "A tabela agrega somente as instâncias que responderem ao recorte atual.",
      liveToggle: "Live",
      liveNavigationWarning: "Desative o modo live para navegar entre as páginas.",
      rowsPerPage: "Itens por página",
      live: "Ao vivo",
      refreshing: "Atualizando...",
      time: "Horário",
      instance: "Instância",
      client: "Cliente",
      domain: "Domínio",
      type: "Tipo",
      status: "Status",
      reply: "Reply",
      upstream: "Upstream",
      details: "Detalhes",
      showing: (start, end, total) => `Mostrando ${start}-${end} de ${total}`,
      previous: "Anterior",
      next: "Próxima",
      noResultsTitle: "Nenhuma query encontrada",
      noResultsDescription: "Ajuste os filtros ou aguarde novas consultas chegarem pelas instâncias selecionadas.",
    },
    details: {
      dnssec: "DNSSEC",
      listId: "List ID",
      ede: "EDE",
      cname: "CNAME",
    },
    actions: {
      block: "Bloquear",
      blockDomain: "Bloquear domínio",
      blockRegex: "Bloquear via regex",
      allow: "Permitir",
      applying: "Aplicando...",
    },
    toasts: {
      actionSuccess: (action, count) =>
        count === 1 ? `${action} aplicado em 1 instância.` : `${action} aplicado em ${count} instâncias.`,
      actionPartial: (action, successCount, failedCount) =>
        `${action} concluído em ${successCount} instâncias, com falha em ${failedCount}.`,
      instanceFailure: (instanceName, message) => `${instanceName}: ${message}`,
      genericInstanceFailure: (instanceName) => `${instanceName}: a ação de domínio falhou.`,
      groupReviewWarning: (reviewCount, failedCount) =>
        failedCount > 0
          ? `${reviewCount} instância(s) precisam de revisão de grupos e ${failedCount} falharam ao atualizar o cache.`
          : `${reviewCount} instância(s) precisam de revisão de grupos para manter os filtros corretos.`,
      reviewGroupsAction: "Abrir /groups",
    },
    statusTypes: {
      cache: "CACHE: resposta servida diretamente do cache.",
      forwarded: "FORWARDED: consulta encaminhada para o servidor upstream.",
      cacheStale: "CACHE_STALE: resposta servida do cache expirado.",
      gravity: "GRAVITY: consulta bloqueada pelo Gravity.",
      unknown: (status) => `${status}: status retornado pela instância Pi-hole.`,
      unavailable: "Status indisponível.",
    },
  },
  clients: {
    eyebrow: "Clientes reconhecidos pela rede do Pi-hole",
    title: "Clients",
    description:
      "Liste dispositivos por MAC, visualize em quais instâncias eles aparecem e aplique grupos em todas elas.",
    table: {
      title: "Clientes cadastrados",
      description: (baselineName) =>
        `A baseline ${baselineName} continua como referência principal para grupos e comentários, enquanto a lista de dispositivos vem das instâncias habilitadas.`,
      tags: "Tags",
      tagsPlaceholder: "Adicionar tag e pressionar Enter",
      tagsDescription: "Use tags locais para organizar clientes e ocultar categorias como IoT sem poluir a tabela.",
      tagRemove: (tag) => `Remover tag ${tag}`,
      hideTags: "Ocultar tags",
      hideTagsDescription: "Selecione tags que devem ser removidas da lista atual de clientes.",
      hideTagsEmpty: "Nenhuma tag local cadastrada até o momento.",
      hiddenTagsSelected: (count) => (count === 1 ? "1 tag oculta" : `${count} tags ocultas`),
      clearHiddenTags: "Limpar ocultação",
      searchPlaceholder: "Pesquisar cliente, IP ou MAC",
      searchClear: "Limpar pesquisa",
      refresh: "Atualizar",
      refreshLoading: "Atualizando...",
      sync: "Sync",
      syncLoading: "Sincronizando...",
      alias: "Cliente",
      instance: "Instância",
      group: "Grupo",
      firstSeen: "Visto primeiro",
      lastQuery: "Última Requisição",
      numQueries: "Total de Queries",
      comments: "Comentários",
      actions: "Ação",
      rowsPerPage: "Itens por página",
      showing: (start, end, total) => `Mostrando ${start}-${end} de ${total}`,
      previous: "Anterior",
      next: "Próxima",
      emptyTitle: "Nenhum cliente encontrado",
      emptyDescription: "As instâncias consultadas não retornaram dispositivos com endereço MAC válido.",
      emptyFilteredTitle: "Nenhum cliente visível com o filtro atual",
      emptyFilteredDescription: "Remova alguma tag oculta para voltar a exibir clientes nesta tabela.",
      instanceDetailsAction: (hwaddr) => `Ver quais instâncias estão enxergando ${hwaddr}`,
      viewMore: "Ver mais",
      editTags: "Editar tags",
      actionPlaceholder: "Ações em breve",
      groupEditorTitle: "Editar grupos",
      groupEditorDescription: "Selecione um ou mais grupos da baseline e salve em todas as instâncias.",
      saveGroups: "Salvar grupos",
      saveGroupsLoading: "Salvando...",
      groupsRequired: "Selecione pelo menos um grupo antes de salvar.",
    },
    dialogs: {
      instancesTitle: (hwaddr) => `Instâncias vendo ${hwaddr}`,
      instancesDescription: "Estas são as instâncias que atualmente enxergam este MAC na tabela de rede.",
      detailsTitle: (hwaddr) => `Detalhes de ${hwaddr}`,
      detailsDescription: "Confira os dados consolidados deste cliente e ajuste nome, tags, grupos e comentários.",
      detailsSave: "Salvar alterações",
      detailsSaveLoading: "Salvando...",
      tagsTitle: (hwaddr) => `Tags de ${hwaddr}`,
      tagsModalDescription: "Edite somente as tags locais deste cliente sem abrir o modal completo.",
      tagsSave: "Salvar tags",
      tagsSaveLoading: "Salvando tags...",
      detailsOverviewTab: "Geral",
      detailsGroupsTab: "Grupos",
      detailsInstancesTab: "Detalhes por instância",
      alias: "Cliente",
      tags: "Tags",
      tagsPlaceholder: "Adicionar tag e pressionar Enter",
      tagsDescription: "Pressione Enter ou vírgula para adicionar uma tag. As sugestões usam tags já existentes.",
      hwaddr: "MAC (HWADDR)",
      ips: "IPs detectados",
      macVendor: "Fabricante MAC",
      preferredInstance: "Instância preferida",
      preferredInstanceId: "ID da instância preferida",
      visibleInstances: "Instâncias visíveis",
      visibleInstanceIds: "IDs das instâncias visíveis",
      selectedGroups: "Grupos selecionados",
      groupSelectionDescription: "Selecione os grupos que devem ser aplicados em todas as instâncias disponíveis.",
      comment: "Comentários",
      totalQueries: "Total de Queries consolidado",
      instanceBreakdownTitle: "Detalhes por instância",
      instanceBreakdownDescription:
        "Aqui ficam separados os dados individuais de cada instância que enxergou este MAC.",
      instanceQueries: "Queries nesta instância",
      instanceId: "ID da instância",
      close: "Fechar",
      syncTitle: "Sincronizar clientes",
      syncDescription: (baselineName) =>
        `Copie os clientes gerenciados da baseline ${baselineName} para as demais instâncias habilitadas.`,
      syncWarning: "Os grupos e comentários da baseline serão aplicados aos destinos disponíveis.",
      syncExtraEntries: "Clientes extras já existentes nas outras instâncias não serão removidos automaticamente.",
      syncCancel: "Cancelar",
      syncConfirm: "Executar sync",
      syncConfirmLoading: "Sincronizando...",
    },
    alerts: {
      partialAvailability: (availableCount, totalCount) =>
        `A leitura conseguiu consultar ${availableCount} de ${totalCount} instâncias habilitadas. As indisponíveis ficaram fora desta tabela.`,
    },
    toasts: {
      refreshFailed: "Não foi possível atualizar a lista de clientes.",
      saveSuccess: "Alterações do cliente salvas com sucesso em todas as instâncias.",
      savePartial: (successfulCount, failedCount) =>
        `${successfulCount} instâncias aplicaram a alteração e ${failedCount} falharam.`,
      syncSuccess: "Sync manual de clientes concluído com sucesso.",
      syncPartial: (successfulCount, failedCount) =>
        `${successfulCount} instâncias foram reconciliadas e ${failedCount} falharam.`,
      instanceFailure: (instanceName, message) => `${instanceName}: ${message}`,
      syncHint: "Se preferir, use o botão Sync para uma reconciliação manual.",
    },
  },
  groups: {
    eyebrow: "Gerenciamento sincronizado de grupos",
    title: "Groups",
    description: "Leia a baseline e sincronize alterações em todas as instâncias gerenciadas.",
    validation: {
      createNames:
        'Informe um ou mais nomes válidos. Separe por espaço ou vírgula e use aspas para nomes com espaços, por exemplo "Meu Novo Grupo".',
      editName: "Informe um nome de grupo válido.",
      comment: "O comentário deve ter no máximo 500 caracteres.",
    },
    create: {
      title: "Adicionar grupos",
      description: "Os novos grupos são criados ativos por padrão e sincronizados em todas as instâncias.",
      nameLabel: "Nome",
      commentLabel: "Comentário",
      tipMultiple: "É possível adicionar vários grupos separando cada nome de grupo com um espaço ou vírgula.",
      tipQuoted: 'Os nomes dos grupos podem conter espaços se forem inseridos entre aspas. Exemplo: "Meu Novo Grupo".',
      submitIdle: "Adicionar grupos",
      submitLoading: "Salvando...",
    },
    table: {
      title: "Groups cadastrados",
      description: (baselineName) =>
        `Visão consolidada das instâncias gerenciadas, usando ${baselineName} como baseline principal.`,
      searchPlaceholder: "Pesquisar groups por nome",
      refresh: "Atualizar",
      refreshLoading: "Atualizando...",
      sync: "Sincronizar",
      syncLoading: "Sincronizando...",
      selectAll: "Selecionar todos os groups visíveis",
      selectRow: (name) => `Selecionar o group ${name}`,
      name: "Nome",
      status: "Status",
      comment: "Comentário",
      actions: "Ação",
      edit: "Editar",
      delete: "Apagar",
      deleteSelected: (count) => (count === 1 ? "Apagar 1 selecionado" : `Apagar ${count} selecionados`),
      protectedBadge: "Protegido",
      protectedDescription: "Group padrão do sistema",
      syncIssue: "Sync pendente",
      syncIssueAction: (name) => `Ver pendências de sincronização do group ${name}`,
      emptyTitle: "Nenhum group encontrado",
      emptyDescription: "Ajuste a busca ou cadastre um novo group para começar.",
    },
    syncDialog: {
      titleAll: "Revisar groups pendentes",
      titleSingle: (name) => `Sincronizar ${name}`,
      descriptionAll: (baselineName) =>
        `Escolha a instância de origem e para quais destinos ausentes copiar cada group. A baseline atual é ${baselineName}.`,
      descriptionSingle: (name) =>
        `Veja em quais instâncias o group ${name} já existe e selecione para onde ele deve ser copiado.`,
      partialAvailability: (availableCount, totalCount) =>
        `A análise foi feita em ${availableCount} de ${totalCount} instâncias habilitadas para sync. As indisponíveis não entram nesta comparação.`,
      emptyTitle: "Nenhum group pendente",
      emptyDescription: "Todos os groups conhecidos já aparecem em todas as instâncias disponíveis para sync.",
      sourceLabel: "Origem",
      sourceHint: (count) => (count === 1 ? "Disponível em 1 instância" : `Disponível em ${count} instâncias`),
      availabilityHint: (presentCount, missingCount) =>
        `${presentCount} instância(s) com o group e ${missingCount} instância(s) sem ele.`,
      sourcePlaceholder: "Selecione a origem",
      targetsLabel: "Destinos",
      targetsHint: (count) => (count === 1 ? "1 destino disponível" : `${count} destinos disponíveis`),
      presentCount: (count) => (count === 1 ? "1 com group" : `${count} com group`),
      missingCount: (count) => (count === 1 ? "1 sem group" : `${count} sem group`),
      instanceHasGroup: "Com group",
      instanceMissingGroup: "Sem group",
      targetsRequired: "Selecione pelo menos uma instância de destino para sincronizar este group.",
      noTargets: "Não há outras instâncias disponíveis para aplicar este group.",
      syncAction: "Sincronizar group",
      syncLoading: "Sincronizando...",
      close: "Fechar",
    },
    status: {
      active: "Ativo",
      inactive: "Inativo",
      enabling: "Ativando...",
      disabling: "Desativando...",
    },
    edit: {
      title: "Editar group",
      description: "Atualize o nome e o comentário e sincronize a alteração em todas as instâncias.",
      nameLabel: "Nome",
      commentLabel: "Comentário",
      cancel: "Cancelar",
      submitIdle: "Salvar alterações",
      submitLoading: "Salvando...",
    },
    members: {
      menuAction: "Ver clientes",
      title: (name) => `Clientes do group ${name}`,
      description: (name) =>
        `Veja os clientes atualmente vinculados ao group ${name} e ajuste os groups de forma individual ou em lote.`,
      close: "Fechar",
      tabs: {
        clients: "Clientes",
        editGroups: "Editar groups",
      },
      clients: {
        title: "Clientes vinculados",
        description: "A lista abaixo mostra apenas clientes que atualmente pertencem a este group.",
        loading: "Carregando clientes vinculados...",
        emptyTitle: "Nenhum cliente vinculado",
        emptyDescription: "Quando houver clientes associados a este group eles aparecerão aqui.",
        selectAll: "Selecionar todos os clientes visíveis",
        selectRow: (client) => `Selecionar o cliente ${client}`,
        alias: "Cliente",
        ip: "IP",
        actions: "Ações",
        editGroups: "Editar groups",
        selectedCount: (count) => (count === 1 ? "1 cliente selecionado" : `${count} clientes selecionados`),
        manageSelected: "Gerenciar selecionados",
      },
      editor: {
        title: (client) => `Editar groups de ${client}`,
        bulkTitle: (count) => (count === 1 ? "Editar groups de 1 cliente" : `Editar groups de ${count} clientes`),
        description: "Atualize os groups deste cliente usando a mesma seleção consolidada do baseline.",
        bulkDescription:
          "Os groups marcados serão aplicados igualmente a todos os clientes selecionados. Os não marcados serão removidos.",
        selectedGroups: "Groups selecionados",
        selectedClients: "Clientes selecionados",
        save: "Salvar groups",
        saving: "Salvando...",
        cancel: "Cancelar",
        groupsRequired: "Selecione pelo menos um group antes de salvar.",
        retryHint: "Algumas alterações falharam. Revise os toasts e tente salvar novamente.",
      },
      bulk: {
        title: "Editar groups em lote",
        description:
          "Aplique diferenças sobre os clientes selecionados sem sobrescrever os demais groups já existentes.",
        emptySelectionTitle: "Selecione clientes para continuar",
        emptySelectionDescription: "Volte para a aba Clientes e marque pelo menos um cliente para editar em lote.",
        selectedClients: "Clientes selecionados",
        addGroups: "Adicionar groups",
        removeGroups: "Remover groups",
        noChanges: "Marque ao menos um group para adicionar ou remover.",
        apply: "Aplicar alterações",
        applying: "Aplicando...",
        groupsRequired: "A alteração deixaria um ou mais clientes sem group. Ajuste a seleção antes de salvar.",
      },
      toasts: {
        loadFailed: "Não foi possível carregar os clientes vinculados a este group.",
        syncRequired: "Sincronize os groups antes de revisar os clientes vinculados.",
        saveSuccess: "Groups do cliente atualizados com sucesso.",
        bulkSaveSuccess: (count) =>
          count === 1 ? "Alterações aplicadas em 1 cliente." : `Alterações aplicadas em ${count} clientes.`,
        bulkSavePartial: (successfulCount, failedCount) =>
          successfulCount === 1 && failedCount === 1
            ? "1 cliente foi atualizado e 1 falhou."
            : `${successfulCount} clientes foram atualizados e ${failedCount} falharam.`,
        bulkSaveFailure: "Não foi possível atualizar os groups dos clientes selecionados.",
        requestFailure: (client, message) => `${client}: ${message}`,
        instanceFailure: (client, instanceName, message) => `${client} · ${instanceName}: ${message}`,
        bulkInstanceFailure: (count, instanceName, message) =>
          count === 1 ? `${instanceName}: ${message}` : `${instanceName} (${count} clientes): ${message}`,
      },
    },
    delete: {
      titleSingle: (name) => `Apagar ${name}?`,
      titleBatch: (count) => (count === 1 ? "Apagar 1 group?" : `Apagar ${count} groups?`),
      descriptionSingle: (name) => `O group ${name} será removido de todas as instâncias gerenciadas.`,
      descriptionBatch: (count) =>
        count === 1
          ? "O group selecionado será removido de todas as instâncias gerenciadas."
          : `${count} groups serão removidos de todas as instâncias gerenciadas.`,
      irreversible: "Essa ação é irreversível.",
      dontAskAgain: "Não lembrar novamente",
      cancel: "Cancelar",
      confirmSingle: "Apagar group",
      confirmBatch: (count) => (count === 1 ? "Apagar 1 group" : `Apagar ${count} groups`),
      confirmLoading: "Apagando...",
    },
    toasts: {
      refreshFailed: "Não foi possível atualizar a lista de groups.",
      createSuccess: "Groups sincronizados com sucesso.",
      updateSuccess: "Group atualizado com sucesso.",
      deleteSuccess: "Groups removidos com sucesso.",
      enabledSuccess: "Group ativado com sucesso.",
      disabledSuccess: "Group desativado com sucesso.",
      syncSuccess: "Sincronismo manual concluído com sucesso.",
      syncGroupSuccess: (name) => `O group ${name} foi sincronizado com sucesso.`,
      partialWarning: (successfulCount, failedCount) =>
        `${successfulCount} instâncias aplicaram a alteração e ${failedCount} falharam ou foram mantidas sem alteração.`,
    },
  },
  lists: {
    eyebrow: "Pi-hole",
    title: "Listas de Bloqueio",
    description: "Gerencie as listas já cadastradas no Pi-hole e sincronize entre instâncias.",
    create: {
      description: "Cadastre uma nova lista e replique a configuração para todas as instâncias gerenciadas.",
      addressLabel: "Endereço",
      commentLabel: "Comentário",
      groupLabel: "Grupos",
      submitLoading: "Salvando...",
    },
    table: {
      title: "Listas gerenciadas",
      description: (baselineName) => `Comparando as listas sincronizáveis a partir da baseline ${baselineName}.`,
      address: "Endereço",
      type: "Tipo",
      status: "Status",
      comment: "Comentários",
      group: "Grupo Designado",
      actions: "Ações",
      edit: "Ver mais",
      delete: "Remover",
      searchPlaceholder: "Filtrar por endereço ou comentário...",
      searchClear: "Limpar busca",
      refresh: "Atualizar",
      refreshLoading: "Atualizando...",
      sync: "Sync",
      syncLoading: "Sincronizando...",
      selectAll: "Selecionar todas",
      selectRow: (address) => `Selecionar lista ${address}`,
      deleteSelected: (count) => (count === 1 ? "Apagar 1 selecionada" : `Apagar ${count} selecionadas`),
      rowsPerPage: "Itens por página",
      showing: (start, end, total) => `Mostrando ${start}-${end} de ${total}`,
      previous: "Anterior",
      next: "Próxima",
      syncHealthy: "Sincronizada",
      syncIssue: "Sync pendente",
      syncIssueAction: (address) => `Revisar pendências de sync da lista ${address}`,
      emptyTitle: "Nenhuma lista encontrada",
      emptyDescription: "Suas adlists aparecerão aqui após a sincronização.",
      addAllow: "Permitir",
      addBlock: "Bloquear",
    },
    alerts: {
      partialAvailability: (availableCount, totalCount) =>
        `A comparação usou ${availableCount} de ${totalCount} instâncias habilitadas para sync. As indisponíveis ficam fora desta análise.`,
    },
    groupEditor: {
      title: "Editar Lista",
      description: "Visualize detalhes e altere as configurações desta lista.",
      save: "Salvar Alterações",
      saving: "Salvando...",
      cancel: "Cancelar",
      groupsRequired: "Selecione pelo menos um grupo.",
      tabs: {
        general: "Geral",
        groups: "Grupos",
      },
      fields: {
        address: "Endereço",
        enabled: "Status",
        dateAdded: "Data de Adição",
        dateModified: "Data de Modificação",
        type: "Tipo",
        lastUpdated: "Última Atualização",
        itemCount: "Quantidade de Itens",
        comment: "Comentário",
      },
    },
    syncDialog: {
      titleAll: "Sincronizar listas",
      titleSingle: (address) => `Sincronizar ${address}`,
      descriptionAll: (baselineName) =>
        `A sincronização manual usa a baseline ${baselineName} como referência e tenta reconciliar as listas entre as instâncias gerenciadas.`,
      descriptionSingle: (address) => `Escolha a origem e os destinos para aplicar manualmente a lista ${address}.`,
      bulkTitle: "Sincronizar todas as listas",
      bulkDescription: (baselineName) =>
        `A baseline ${baselineName} será usada como referência para reconciliar as listas conhecidas entre as instâncias gerenciadas.`,
      partialAvailability: (availableCount, totalCount) =>
        `A análise foi feita em ${availableCount} de ${totalCount} instâncias habilitadas para sync. As indisponíveis não entram nesta comparação.`,
      sourceLabel: "Origem",
      availabilityHint: (presentCount, missingCount) =>
        `${presentCount} instância(s) com a lista e ${missingCount} instância(s) sem ela.`,
      sourcePlaceholder: "Selecione a origem",
      targetsLabel: "Destinos",
      instanceHasItem: "Com lista",
      instanceMissingItem: "Sem lista",
      targetsRequired: "Selecione pelo menos uma instância de destino para sincronizar esta lista.",
      noTargets: "Não há outras instâncias disponíveis para aplicar esta lista.",
      syncAction: "Sincronizar lista",
      syncLoading: "Sincronizando...",
      confirmBulk: "Sincronizar tudo",
      close: "Fechar",
    },
    status: {
      enabled: "Ativo",
      disabled: "Inativo",
      enabling: "Ativando...",
      disabling: "Desativando...",
    },
    toasts: {
      refreshFailed: "Não foi possível atualizar a lista de listas.",
      createSuccess: "Lista criada com sucesso em todas as instâncias.",
      updateSuccess: "Lista atualizada com sucesso em todas as instâncias.",
      updatePartial: (successCount, failedCount) =>
        `Atualização parcial: ${successCount} sucessos, ${failedCount} falhas.`,
      deleteSuccess: "Listas removidas com sucesso.",
      enabledSuccess: "Lista ativada com sucesso.",
      disabledSuccess: "Lista desativada com sucesso.",
      syncSuccess: "Sincronização manual concluída com sucesso.",
      syncItemSuccess: (address) => `A lista ${address} foi sincronizada com sucesso.`,
      partialWarning: (successfulCount, failedCount) =>
        `${successfulCount} instâncias aplicaram a alteração e ${failedCount} falharam ou foram mantidas sem alteração.`,
      instanceFailure: (instanceName, message) => `${instanceName}: ${message}`,
      syncHint: "Se necessário, use o botão de sync para uma reconciliação manual completa.",
    },
    defaultComment: "Adicionado via YAPD",
    delete: {
      titleSingle: (address) => `Remover ${address}?`,
      titleBatch: (count) => (count === 1 ? "Remover 1 lista?" : `Remover ${count} listas?`),
      descriptionSingle: (address) => `A lista ${address} será removida de todas as instâncias gerenciadas.`,
      descriptionBatch: (count) =>
        count === 1
          ? "A lista selecionada será removida de todas as instâncias gerenciadas."
          : `${count} listas serão removidas de todas as instâncias gerenciadas.`,
      irreversible: "Essa ação é irreversível.",
      dontAskAgain: "Não lembrar novamente",
      cancel: "Cancelar",
      confirmSingle: "Remover lista",
      confirmBatch: (count) => (count === 1 ? "Remover 1 lista" : `Remover ${count} listas`),
      confirmLoading: "Removendo...",
    },
  },
  domains: {
    eyebrow: "Pi-hole",
    title: "Domínios",
    description: "Gerencie domínios exatos e regex pré-definidos em todas as instâncias.",
    create: {
      description:
        "Cadastre novos domínios exatos ou regex e replique a configuração para todas as instâncias gerenciadas.",
      domainLabel: "Domínio / Regex",
      commentLabel: "Comentário",
      groupLabel: "Designar grupos",
      kindLabel: "Tipo de filtro",
      submitLoading: "Salvando...",
      kinds: {
        exact: {
          label: "Domínio exato",
          description: "Cria uma entrada exata usando exatamente o domínio informado.",
        },
        regexSpecific: {
          label: "Regex: TLD específico",
          description: "Cria um regex para o nome informado com um TLD específico, como .com.",
        },
        regexAny: {
          label: "Regex: qualquer TLD",
          description: "Cria um regex para o nome informado aceitando múltiplos TLDs.",
        },
      },
    },
    table: {
      title: "Domínios gerenciados",
      description: (baselineName) => `Comparando os domínios sincronizáveis a partir da baseline ${baselineName}.`,
      domain: "Domínio / Regex",
      type: "Tipo",
      kind: "Formato",
      status: "Status",
      comment: "Comentários",
      group: "Grupo Designado",
      actions: "Ações",
      edit: "Ver mais",
      delete: "Remover",
      searchPlaceholder: "Filtrar por domínio ou comentário...",
      searchClear: "Limpar busca",
      refresh: "Atualizar",
      refreshLoading: "Atualizando...",
      sync: "Sync",
      syncLoading: "Sincronizando...",
      selectAll: "Selecionar todos",
      selectRow: (domain) => `Selecionar domínio ${domain}`,
      deleteSelected: (count) => (count === 1 ? "Apagar 1 selecionado" : `Apagar ${count} selecionados`),
      rowsPerPage: "Itens por página",
      showing: (start, end, total) => `Mostrando ${start}-${end} de ${total}`,
      previous: "Anterior",
      next: "Próxima",
      syncHealthy: "Sincronizado",
      syncIssue: "Sync pendente",
      syncIssueAction: (domain) => `Revisar pendências de sync do domínio ${domain}`,
      emptyTitle: "Nenhum domínio encontrado",
      emptyDescription: "Domínios gerenciados aparecerão aqui após a sincronização.",
      addAllow: "Permitir",
      addDeny: "Bloquear",
    },
    alerts: {
      partialAvailability: (availableCount, totalCount) =>
        `A comparação usou ${availableCount} de ${totalCount} instâncias habilitadas para sync. As indisponíveis ficam fora desta análise.`,
    },
    filters: {
      exactAllow: "Permitir Exato",
      regexAllow: "Permitir Regex",
      exactDeny: "Bloquear Exato",
      regexDeny: "Bloquear Regex",
    },
    summary: {
      total: "Resumo de domínios",
      allowed: "Permitidos",
      blocked: "Bloqueados",
      exact: "Exato",
      regex: "Regex",
    },
    groupEditor: {
      title: "Editar domínio",
      description: "Visualize detalhes e altere os grupos e comentários deste domínio.",
      save: "Salvar alterações",
      saving: "Salvando...",
      cancel: "Cancelar",
      groupsRequired: "Selecione pelo menos um grupo.",
      tabs: {
        general: "Geral",
        groups: "Grupos",
      },
      fields: {
        domain: "Domínio / Regex",
        enabled: "Status",
        dateAdded: "Data de adição",
        dateModified: "Data de modificação",
        type: "Tipo",
        kind: "Formato",
        comment: "Comentário",
      },
    },
    syncDialog: {
      titleAll: "Sincronizar domínios",
      titleSingle: (domain) => `Sincronizar ${domain}`,
      descriptionAll: (baselineName) =>
        `A sincronização manual usa a baseline ${baselineName} como referência e tenta reconciliar os domínios entre as instâncias gerenciadas.`,
      descriptionSingle: (domain) => `Escolha a origem e os destinos para aplicar manualmente o domínio ${domain}.`,
      bulkTitle: "Sincronizar todos os domínios",
      bulkDescription: (baselineName) =>
        `A baseline ${baselineName} será usada como referência para reconciliar os domínios conhecidos entre as instâncias gerenciadas.`,
      partialAvailability: (availableCount, totalCount) =>
        `A análise foi feita em ${availableCount} de ${totalCount} instâncias habilitadas para sync. As indisponíveis não entram nesta comparação.`,
      sourceLabel: "Origem",
      availabilityHint: (presentCount, missingCount) =>
        `${presentCount} instância(s) com o domínio e ${missingCount} instância(s) sem ele.`,
      sourcePlaceholder: "Selecione a origem",
      targetsLabel: "Destinos",
      instanceHasItem: "Com domínio",
      instanceMissingItem: "Sem domínio",
      targetsRequired: "Selecione pelo menos uma instância de destino para sincronizar este domínio.",
      noTargets: "Não há outras instâncias disponíveis para aplicar este domínio.",
      syncAction: "Sincronizar domínio",
      syncLoading: "Sincronizando...",
      confirmBulk: "Sincronizar tudo",
      close: "Fechar",
    },
    status: {
      enabled: "Ativo",
      disabled: "Inativo",
      enabling: "Ativando...",
      disabling: "Desativando...",
    },
    csv: {
      export: "Exportar CSV",
      exportLoading: "Exportando...",
      import: "Importar CSV",
      importTitle: "Importar domínios por CSV",
      importDescription: "Envie um CSV exportado do YAPD para cadastrar ou atualizar domínios em lote.",
      importFileLabel: "Arquivo CSV",
      importHint: "Colunas esperadas: domain, type, kind, enabled, comment, group.",
      importErrorsLabel: "Erros encontrados",
      importErrorsPlaceholder: "Nenhum erro de importação até agora.",
      importSubmit: "Importar domínios",
      importSubmitting: "Importando...",
      importClose: "Fechar",
      fileRequired: "Selecione um arquivo CSV para importar.",
    },
    delete: {
      titleSingle: (domain) => `Remover ${domain}?`,
      titleBatch: (count) => (count === 1 ? "Remover 1 domínio?" : `Remover ${count} domínios?`),
      descriptionSingle: (domain) => `O domínio ${domain} será removido de todas as instâncias gerenciadas.`,
      descriptionBatch: (count) =>
        count === 1
          ? "O domínio selecionado será removido de todas as instâncias gerenciadas."
          : `${count} domínios serão removidos de todas as instâncias gerenciadas.`,
      irreversible: "Essa ação é irreversível.",
      dontAskAgain: "Não lembrar novamente",
      cancel: "Cancelar",
      confirmSingle: "Remover domínio",
      confirmBatch: (count) => (count === 1 ? "Remover 1 domínio" : `Remover ${count} domínios`),
      confirmLoading: "Removendo...",
    },
    toasts: {
      refreshFailed: "Não foi possível atualizar a lista de domínios.",
      createSuccess: "Domínio criado com sucesso em todas as instâncias.",
      updateSuccess: "Domínio atualizado com sucesso em todas as instâncias.",
      updatePartial: (successCount, failedCount) =>
        `Atualização parcial: ${successCount} sucessos, ${failedCount} falhas.`,
      deleteSuccess: "Domínios removidos com sucesso.",
      enabledSuccess: "Domínio ativado com sucesso.",
      disabledSuccess: "Domínio desativado com sucesso.",
      syncSuccess: "Sincronização manual concluída com sucesso.",
      syncItemSuccess: (domain) => `O domínio ${domain} foi sincronizado com sucesso.`,
      partialWarning: (successfulCount, failedCount) =>
        `${successfulCount} instâncias aplicaram a alteração e ${failedCount} falharam ou foram mantidas sem alteração.`,
      instanceFailure: (instanceName, message) => `${instanceName}: ${message}`,
      syncHint: "Se necessário, use o botão de sync para uma reconciliação manual completa.",
      exportSuccess: "CSV exportado com sucesso.",
      importSuccess: (createdCount, updatedCount) =>
        `Importação concluída: ${createdCount} criados e ${updatedCount} atualizados.`,
      importPartial: (createdCount, updatedCount, invalidCount) =>
        `Importação concluída com pendências: ${createdCount} criados, ${updatedCount} atualizados e ${invalidCount} inválidos.`,
    },
    defaultComment: "Adicionado via YAPD",
  },
  browserExtension: {
    eyebrow: "Extensão",
    title: "YAPD Inspector",
    description: "Gerencie pareamento, dispositivos, configurações e histórico da extensão.",
    pairing: {
      title: "Pareamento",
      description: "Gere um código temporário para conectar uma extensão ao YAPD.",
      generate: "Gerar código",
      generating: "Gerando...",
      code: "Código",
      expiresAt: "Expira em",
      empty: "Nenhum código ativo nesta tela.",
    },
    devices: {
      title: "Dispositivos pareados",
      description: "Tokens ativos e revogados da extensão.",
      name: "Nome",
      browser: "Navegador",
      version: "Versão",
      manifest: "Manifest",
      lastSeen: "Último uso",
      status: "Status",
      actions: "Ações",
      active: "Ativo",
      revoked: "Revogado",
      revoke: "Revogar",
      revoking: "Revogando...",
      empty: "Nenhum dispositivo pareado.",
    },
    settings: {
      title: "Configurações",
      description: "Preferências enviadas para a extensão durante a sincronização de cache.",
      sendPageTitle: "Enviar título da página",
      hardBlocked: "URLs bloqueadas para inspeção",
      sensitive: "URLs sensíveis",
      patternsPlaceholder: "Um padrão por linha",
      save: "Salvar configurações",
      saving: "Salvando...",
    },
    history: {
      title: "Histórico",
      description: "Itens detectados pela extensão consolidados por domínio principal.",
      date: "Data",
      pageDomain: "Página",
      count: "Itens",
      blocked: "Bloqueados",
      notBlocked: "Não bloqueados",
      status: "Status",
      items: "Aplicados",
      errors: "Erros",
      undone: "Desfeito",
      details: "Detalhes",
      hideDetails: "Ocultar",
      empty: "Nenhum item detectado.",
      target: "Target",
      type: "Tipo",
      kind: "Formato",
      category: "Categoria",
      score: "Score",
      reasons: "Motivos",
      evidence: "Evidência",
    },
    toasts: {
      pairingCreated: "Código de pareamento criado.",
      pairingFailed: "Não foi possível gerar o código de pareamento.",
      settingsSaved: "Configurações salvas.",
      settingsFailed: "Não foi possível salvar as configurações.",
      deviceRevoked: "Dispositivo revogado.",
      revokeFailed: "Não foi possível revogar o dispositivo.",
      refreshFailed: "Não foi possível atualizar os dados do Inspector.",
    },
  },
  config: {
    eyebrow: "Configuração central",
    title: "Configurações",
    description: "Edite a configuração detalhada do Pi-hole por aba e sincronize tópicos entre instâncias.",
    sourceLabel: "Instância de origem",
    sourcePlaceholder: "Selecione a origem",
    teleporterDownload: "Baixar Teleporter ZIP",
    teleporterDownloading: "Baixando ZIP...",
    unavailableTitle: "Nem todas as instâncias responderam",
    unavailableDescription: (unavailableCount, totalCount) =>
      `${unavailableCount} de ${totalCount} instâncias habilitadas para sync não puderam ser consultadas.`,
    driftTitle: "Existem tópicos fora de sincronia",
    driftDescription: (driftedCount) =>
      driftedCount === 1 ? "1 item está fora de sincronia." : `${driftedCount} itens estão fora de sincronia.`,
    driftEmpty: "Nenhum item divergente restante.",
    driftLinkLabel: (topicTitle, fieldPath) => `${topicTitle} -> ${fieldPath}`,
    synced: "Sincronizado",
    drifted: "Divergente",
    partial: "Parcial",
    fieldPath: "Caminho",
    fieldType: "Tipo",
    fieldAllowed: "Permitido",
    fieldDefault: "Default",
    fieldDescription: "Descrição",
    fieldFlags: "Flags",
    restartDnsmasq: "Reinicia DNSMasq",
    sessionReset: "Reseta sessão",
    envVar: "Ligado a env var",
    noDescription: "Sem descrição adicional.",
    save: "Salvar",
    saving: "Salvando...",
    discard: "Descartar",
    sync: "Sincronizar",
    syncing: "Sincronizando...",
    editSourceHint: "As alterações desta aba serão aplicadas somente na instância de origem selecionada.",
    jsonHint: "Edite valores não booleanos em texto puro ou JSON válido, conforme o tipo do campo.",
    booleanActive: "Ativo",
    booleanInactive: "Inativo",
    unsavedChanges: "Esta aba tem alterações não salvas. Deseja descartá-las para trocar de aba?",
    saveSuccess: "A aba foi salva com sucesso.",
    ignoreSuccess: "O item foi removido do aviso de sincronismo.",
    unignoreSuccess: "O item voltou para o cálculo de sincronismo.",
    syncSuccess: (count) =>
      count === 1
        ? "1 instância foi sincronizada com sucesso."
        : `${count} instâncias foram sincronizadas com sucesso.`,
    partialSync: (successfulCount, failedCount) =>
      `Sincronização parcial: ${successfulCount} sucesso(s) e ${failedCount} falha(s).`,
    loadFailed: "Não foi possível atualizar as configurações.",
    downloadFailed: "Não foi possível baixar o Teleporter ZIP.",
    empty: "Nenhum campo disponível nesta aba.",
    ignored: "Ignorado",
    ignore: "Ignorar sincronismo",
    ignoring: "Ignorando...",
    unignore: "Voltar a sincronizar",
    unignoring: "Removendo exceção...",
    focusInvalid: "O item solicitado não existe mais nesta aba.",
    syncDialog: {
      title: (topicTitle) => `Sincronizar ${topicTitle}`,
      description: "Escolha a instância de origem e as instâncias de destino para replicar esta aba.",
      sourceLabel: "Origem",
      sourcePlaceholder: "Selecione a origem",
      targetsLabel: "Destinos",
      targetsRequired: "Selecione pelo menos uma instância de destino.",
      close: "Fechar",
      confirm: "Sincronizar aba",
      confirming: "Sincronizando aba...",
      unavailable: "Indisponível",
      baselineBadge: "Baseline",
    },
  },
};
