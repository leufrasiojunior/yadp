...
const setupCopy: Record<AppLocale, SetupCopy> = {
  "pt-BR": {
    ...
    piholes: {
      ...
      description:
        "Adicione IPs ou hosts que o YAPD deve validar no setup, escolha uma senha compartilhada ou individual e defina qual deles será o master.",
      ...
      scheme: "Protocolo",
      url: "IP ou host",
      urlPlaceholder: "192.168.31.17:444",
      urlDescription:
        "Cole somente IP/host e porta. Se colar uma URL completa do Pi-hole, o YAPD remove o caminho automaticamente.",
      ...
      masterDescription: "Selecione qual instância será usada como baseline principal do YAPD.",
      masterPlaceholder: "Selecione o Pi-hole master",
      rowTitle: (index, name) => (name && name.trim().length > 0 ? name : `Pi-hole ${index + 1}`),
      addInstance: "Adicionar Pi-hole",
      removeInstance: "Remover",
      validation: {
        urlFormat: "Informe apenas IP/host e porta válidos. URLs completas são convertidas automaticamente.",
        masterRequired: "Selecione um Pi-hole master válido para continuar.",
        aliasRequired: "Informe um alias para esta instância.",
        urlRequired: "Informe o IP ou host desta instância.",
        instanceIncomplete: (instance) =>
          `Complete alias, IP/host e senha de ${instance} ou deixe essa linha totalmente vazia.`,
  ...
  "en-US": {
    ...
    piholes: {
      ...
      description:
        "Add the IPs or hosts YAPD should validate during setup, choose a shared or per-instance password, and define which one is the master.",
      ...
      urlPlaceholder: "192.168.31.17:444",
      urlDescription:
        "Paste only IP/host and port. If you paste a full Pi-hole URL, YAPD removes the path automatically.",
      ...
      masterDescription: "Select which instance becomes the primary YAPD baseline.",
      masterPlaceholder: "Select the master Pi-hole",
      rowTitle: (index, name) => (name && name.trim().length > 0 ? name : `Pi-hole ${index + 1}`),
      addInstance: "Add Pi-hole",
      removeInstance: "Remove",
      validation: {
        urlFormat: "Provide only a valid IP/host and port. Full URLs are converted automatically.",
        masterRequired: "Select a valid master Pi-hole before continuing.",
        aliasRequired: "Provide an alias for this instance.",
        urlRequired: "Provide the IP or host for this instance.",
        instanceIncomplete: (instance) =>
          `Complete the alias, IP/host, and password for ${instance}, or leave that row entirely empty.`,
